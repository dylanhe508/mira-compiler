 ...
 * 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟?*/
static bool import_is_active(const char *resolved) {
	if (g_current_compiler && g_current_compiler->filename &&
	    strcmp(g_current_compiler->filename, resolved) == 0)
		return true;
	for (LexerState *state = g_current_compiler ? g_current_compiler->lex_state : NULL;
	     state; state = state->prev)
		if (state->filename && strcmp(state->filename, resolved) == 0)
			return true;
	return false;
}
void parser_do_import(const char *path, const char *alias, int is_lib) {
	...
	if (is_lib == 2) {
		size_t need = strlen(exe_dir) + strlen(path) + 24;
		resolved = malloc(need);
#ifdef _WIN32
		snprintf(resolved, need, "%sstdlib\\%s.mira", exe_dir, path);
		...
		if (!tf) {
			mira_error_simple(1, "module '%s' not found at '%s'", path, resolved);
			free(resolved);
			return;
		}
		fclose(tf);
		...
		size_t module_path_size = strlen(path) + 6;
		char *module_path = malloc(module_path_size);
		snprintf(module_path, module_path_size, "%s.mira", path);
		resolved = resolve_path(cur_dir, exe_dir, module_path);
#ifdef _WIN32
		for (char *p = resolved; *p; ++p) if (*p == '/') *p = '\\';
#endif
		free(module_path);
	...
	ModuleId imported_module = g_current_compiler->current_module;
	if (is_lib == 2 || is_lib == 3)
		imported_module = module_register_import(g_current_compiler, path, alias);
	if (import_is_active(resolved)) {
		mira_error_simple(1, "module import cycle involving '%s'", resolved);
		free(resolved);
		return;
	}
	...
	if (!lexer_push_file(resolved, alias)) {
		...
		g_current_compiler->current_module = imported_module;
		g_current_compiler->current_is_stdlib = (is_lib == 2);
	}
	free(resolved);
}
