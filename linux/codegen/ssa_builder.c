...
static const BuiltinWord extern_words[] = {
    // Data list ops
    {"list-new", "mira_list_new", 1, 1, SSA_TYPE_PTR},
    {"list-len", "mira_list_len", 1, 1, SSA_TYPE_INT},
    {"list-get", "mira_list_get", 2, 1, SSA_TYPE_INT},
    {"list-set", "mira_list_set", 3, 0, SSA_TYPE_VOID},
    {"list-free", "mira_list_free", 1, 0, SSA_TYPE_VOID},
    {"list-push", "mira_list_push", 2, 1, SSA_TYPE_PTR},
    // String ops
    {"str-len", "mira_str_len", 1, 1, SSA_TYPE_INT},
    {"str-cat", "mira_str_concat", 2, 1, SSA_TYPE_PTR},
    {"str-concat", "mira_str_concat", 2, 1, SSA_TYPE_PTR},
    {"str-eq", "mira_str_eq", 2, 1, SSA_TYPE_INT},
    {"str-sub", "mira_str_substr", 3, 1, SSA_TYPE_PTR},
    {"str-find", "mira_str_contains", 2, 1, SSA_TYPE_INT},
    {"str-contains", "mira_str_contains", 2, 1, SSA_TYPE_INT},
    {"str-trim", "mira_str_trim", 1, 1, SSA_TYPE_PTR},
    {"str-at", "mira_str_at", 2, 1, SSA_TYPE_INT},
    {"to-int", "mira_str_to_int", 1, 1, SSA_TYPE_INT},
    {"str->int", "mira_str_to_int", 1, 1, SSA_TYPE_INT},
    {"to-str", "mira_to_str", 1, 1, SSA_TYPE_PTR},
    {"int->str", "mira_to_str", 1, 1, SSA_TYPE_PTR},
    {"to-float", "mira_int_to_float", 1, 1, SSA_TYPE_FLOAT},
    {"cr", "mira_cr", 0, 0, SSA_TYPE_VOID},
    {"nl", "mira_cr", 0, 0, SSA_TYPE_VOID},
    // File ops
    {"file-read", "mira_file_read", 1, 1, SSA_TYPE_INT},
    {"file-write", "mira_file_write", 2, 0, SSA_TYPE_VOID},
    {"file-append", "mira_file_append", 2, 0, SSA_TYPE_VOID},
    {"file-exists", "mira_file_exists", 1, 1, SSA_TYPE_INT},
    {"file-delete", "mira_file_delete", 1, 0, SSA_TYPE_VOID},
    // Math ops
    {"abs", "mira_abs", 1, 1, SSA_TYPE_INT},
    {"min", "mira_min", 2, 1, SSA_TYPE_INT},
    {"max", "mira_max", 2, 1, SSA_TYPE_INT},
    {"sqrt", "mira_f_sqrt", 1, 1, SSA_TYPE_FLOAT},
    {"pow", "mira_f_pow", 2, 1, SSA_TYPE_FLOAT},
    {"rand", "mira_random", 0, 1, SSA_TYPE_INT},
    {"srand", "mira_random_seed", 1, 0, SSA_TYPE_VOID},
    // IO ops
    {"read", "mira_read_int", 0, 1, SSA_TYPE_INT},
    {"input", "mira_input", 0, 1, SSA_TYPE_INT},
    // WinAPI ops
    {"msgbox", "mira_win_msgbox", 2, 0, SSA_TYPE_VOID},
    {"sleep", "mira_win_sleep", 1, 0, SSA_TYPE_VOID},
    {"shell", "mira_win_shell", 1, 1, SSA_TYPE_INT},
    {"env", "mira_win_env", 1, 1, SSA_TYPE_INT},
    {"clipboard", "mira_win_clip_get", 0, 1, SSA_TYPE_INT},
    {"pid", "mira_win_pid", 0, 1, SSA_TYPE_INT},
    {"clock", "mira_win_tick", 0, 1, SSA_TYPE_INT},
    {"clock-ns", "mira_win_tick_ns", 0, 1, SSA_TYPE_INT},
    // Async ops
    {"spawn", "mira_async_start", 1, 0, SSA_TYPE_VOID},
    {"wait", "mira_async_yield", 0, 0, SSA_TYPE_VOID},
    {"parallel", "mira_parallel_start", 1, 1, SSA_TYPE_INT},
    {"join", "mira_parallel_join", 1, 0, SSA_TYPE_VOID},
    {"go", "mira_go_start0", 1, 1, SSA_TYPE_INT},
    {"join-task", "mira_go_join", 1, 0, SSA_TYPE_VOID},
    {"yield-task", "mira_go_yield", 0, 0, SSA_TYPE_VOID},
    {"wait-all", "mira_go_wait_all", 0, 0, SSA_TYPE_VOID},
    {"channel", "mira_channel_new_value", 1, 1, SSA_TYPE_PTR},
    {"send", "mira_channel_send_value", 2, 1, SSA_TYPE_INT},
    {"recv", "mira_channel_recv_value", 1, 1, SSA_TYPE_INT},
    {"close-channel", "mira_channel_close_value", 1, 1, SSA_TYPE_INT},
    {"free-channel", "mira_channel_free_value", 1, 0, SSA_TYPE_VOID},
    {"throw", "mira_throw", 1, 0, SSA_TYPE_VOID},
    {"get-error", "mira_get_error", 0, 1, SSA_TYPE_INT},
    // Memory ops
    {"allocate", "mem_alloc", 1, 1, SSA_TYPE_INT},
    {"free", "mem_free", 1, 0, SSA_TYPE_VOID},
    {"move", "mem_move", 3, 0, SSA_TYPE_VOID},
    {"erase", "mem_erase", 2, 0, SSA_TYPE_VOID},
    {"dump", "mira_mem_dump", 2, 0, SSA_TYPE_VOID},
    {"md", "mira_mem_dump", 2, 0, SSA_TYPE_VOID},
};
...
static void build_op(SsaBuilderCtx *ctx, IrNode *o, IrNode *next) {
	...
	case IR_VAR: {
		...
		if (next && next->kind == IR_WORD) {
			...
			if (n_len == 1 && n[0] == '!') {
				...
				VReg dst = ssa_new_vreg(ctx->cur_func, SSA_TYPE_INT);
				SsaInst *inst = alloc_inst(ctx->cur_block, SSA_OP_LOAD_VAR, SSA_TYPE_INT, dst);
	...
	case IR_WORD: {
		...
		if(ISW("true")) {
			...
			SsaType lt = (l > 0 && ctx->cur_func->vreg_defs[l]) ? ctx->cur_func->vreg_defs[l]->type : SSA_TYPE_INT;
			SsaType rt = (r > 0 && ctx->cur_func->vreg_defs[r]) ? ctx->cur_func->vreg_defs[r]->type : SSA_TYPE_INT;
			...
			VReg dst = ssa_new_vreg(ctx->cur_func, SSA_TYPE_INT);
			SsaInst *i = alloc_inst(ctx->cur_block, SSA_OP_CALL, SSA_TYPE_INT, dst);
			i->operand_cap = 1;
			i->operands = malloc(sizeof(SsaOperand) * 1);
			i->operands[0].kind = SSA_OPND_SYM;
			i->operands[0].u.sym = strdup("mira_time_ms");
			i->operand_count = 1;
			push_vreg(ctx, dst);
		}
		else if(ISW("clock-ns")) {
			VReg dst = ssa_new_vreg(ctx->cur_func, SSA_TYPE_INT);
			SsaInst *i = alloc_inst(ctx->cur_block, SSA_OP_CALL, SSA_TYPE_INT, dst);
			i->operand_cap = 1;
			i->operands = malloc(sizeof(SsaOperand));
			i->operands[0].kind = SSA_OPND_SYM;
			i->operands[0].u.sym = strdup("mira_win_tick_ns");
			i->operand_count = 1;
			push_vreg(ctx, dst);
		}
			...
			bool found = false;
			for (size_t wi = 0; wi < sizeof(extern_words)/sizeof(extern_words[0]); wi++) {
				if (strcmp(sym, extern_words[wi].name) == 0) {
					const BuiltinWord *bw = &extern_words[wi];
					
					VReg dst = 0;
					if (bw->ret) {
						dst = ssa_new_vreg(ctx->cur_func, bw->ret_type);
					}
					
					SsaInst *i = alloc_inst(ctx->cur_block, SSA_OP_CALL, bw->ret ? bw->ret_type : SSA_TYPE_VOID, dst);
					i->operand_cap = bw->args + 1; // sym + args
					i->operands = malloc(sizeof(SsaOperand) * i->operand_cap);
					
					i->operands[0].kind = SSA_OPND_SYM;
					i->operands[0].u.sym = strdup(bw->c_func);
					i->operand_count = 1;
					
					// Pop arguments in reverse order (stack semantics)
					if (bw->args > 0) {
						VReg *args = malloc(sizeof(VReg) * bw->args);
						for (int a = 0; a < bw->args; a++) {
							args[a] = pop_vreg(ctx);
						}
						// First pop is lIR argument
						for (int a = bw->args - 1; a >= 0; a--) {
					...
					if (bw->ret) {
						...
						if (strcmp(bw->c_func, "mem_alloc") == 0) {
							i->needs_free = 1;
							i->free_func_name = "mem_free";
						} else if (strcmp(bw->c_func, "mira_list_new") == 0 ||
								   strcmp(bw->c_func, "mira_list_push") == 0) {
							i->needs_free = 1;
							i->free_func_name = "mira_list_free";
						} else if (strcmp(bw->c_func, "mira_str_concat") == 0 ||
								   strcmp(bw->c_func, "mira_to_str") == 0 ||
								   strcmp(bw->c_func, "mira_str_substr") == 0) {
							i->needs_free = 1;
							i->free_func_name = "mem_free";
						}
					}
			...
			if (!found) {
				...
				if (is_lambda_param) {
					...
					if (!found_def && var_slot >= 0) {
						...
						SsaType call_type = SSA_TYPE_INT;
						...
						VReg dst = ssa_new_vreg(ctx->cur_func, call_type);
	...
	case IR_IF: {
		...
		if (else_b) {
			...
			for (int slot = 0; slot < then_depth; slot++) {
