...
static int ssa_internal_slot_limit = -1;
...
bool ssa_opt_induction_strength_reduce(SsaFunction *func) {
    ...
    for (int li = 0; li < func->loop_count; ++li) {
            ...
            for (SsaInst *inst = func->blocks[bi]->inst_head; inst;
                ...
                if (candidate_value != induction_value &&
                    ...
                    !vreg_is_induction_phi(func, candidate_value, loop,
                                           ...
                                           update_store))
                    continue;
                if (!candidate_count) factor = current_factor;
                if (current_factor != factor || candidate_count >= 64) continue;
                candidates[candidate_count++] = inst;
            }
        }
        if (!candidate_count) continue;
        /* One derived recurrence stays live for the whole loop.  The generic
         * pre-pass pressure estimate understates the post-inline peak, so use
         * a deliberately conservative local gate until recurrence-aware
         * coloring can model that extra live range exactly. */
        if (func->estimated_scalar_pressure >= 10 ||
            loop->decision_plan.body_instructions > 64)
            continue;

        if (ssa_internal_next_slot < 0 ||
            ssa_internal_next_slot >= ssa_internal_slot_limit)
            continue;
        ...
        SsaInst *derived_update_store = new_ssa_inst(func, SSA_OP_STORE_VAR,
            SSA_TYPE_VOID, 0);
        ...
        insert_inst_before(loop->latch, loop->latch->inst_tail, derived_load);
        insert_inst_before(loop->latch, loop->latch->inst_tail, derived_add);
        insert_inst_before(loop->latch, loop->latch->inst_tail,
                           derived_update_store);
        for (int ci = 0; ci < candidate_count; ++ci) {
            candidates[ci]->IrNode = SSA_OP_LOAD_VAR;
            candidates[ci]->op1.kind = SSA_OPND_IMM;
            candidates[ci]->op1.u.imm = derived_slot;
            candidates[ci]->op2.kind = SSA_OPND_NONE;
            candidates[ci]->operand_count = 1;
        }
        changed = true;
    }
    return changed;
    ...
    for (int fi = 0; fi < mod->func_count; ++fi)
        for (int bi = 0; bi < mod->functions[fi]->block_count; ++bi)
            for (SsaInst *inst = mod->functions[fi]->blocks[bi]->inst_head;
                ...
                if (slot >= source_slot_count) source_slot_count = slot + 1;
            }
    ssa_internal_next_slot = source_slot_count;
    ssa_internal_slot_limit = source_slot_count + 16;
    for (int fi = 0; fi < mod->func_count; ++fi)
