...
static double codegen_profile_ms(clock_t begin, clock_t end) {
...
static const char *externs[] = {
	"mira_float_tmp",
	"mira_print", "mira_read_int", "mira_input", "ExitProcess", "exit", "mira_cr",
...
void codegen(Compiler *c, Program *prog) {
	...
	clock_t codegen_profile_ticks[9] = {0};
	int codegen_profile_enabled = getenv("MIRA_COMPILE_PROFILE") != NULL;
#define CODEGEN_PROFILE_TICK(index) do { \
	if (codegen_profile_enabled) codegen_profile_ticks[(index)] = clock(); \
} while (0)
	CODEGEN_PROFILE_TICK(0);

	// ??????IR -> SSA TAC (?????Alloca)
	extern void ssa_build_program(Program *prog, SsaModule *out_mod);
	ssa_build_program(prog, &ssa_mod);
	CODEGEN_PROFILE_TICK(1);

	/* ???:?? CFG??????PHI,??? mem2reg? */
	extern void ssa_build(SsaModule *mod);
	ssa_build(&ssa_mod);
	CODEGEN_PROFILE_TICK(2);

	/* Build ownership/alias/effect facts on the finalized SSA closure. */
	ssa_ref_analyze_module(&ssa_mod);
	CODEGEN_PROFILE_TICK(3);

	/* ???:?? SSA ???????????,????????
	 * ?????????? */
	extern void ssa_optimize_module(SsaModule *mod);
	ssa_optimize_module(&ssa_mod);
	CODEGEN_PROFILE_TICK(4);

	/* Internal recurrence slots are created during SSA optimization.  Emit the
	 * backing store only after the exact module-wide high-water mark is known. */
	int final_var_slots = ssa_mod.var_slot_count;
	if (final_var_slots < prog->var_count) final_var_slots = prog->var_count;
	if (final_var_slots < 1) final_var_slots = 1;
	ir_bss_label(&cg->ir, "mira_vars");
	ir_bss_resq(&cg->ir, final_var_slots);
	...
	ssa_compute_var_reg_maps(&ssa_mod, final_var_slots);
	CODEGEN_PROFILE_TICK(5);

	/* ???:? SSA VReg ?????? x86-64/YMM ???????? */
	extern void ssa_allocate_registers(SsaModule *mod);
	ssa_allocate_registers(&ssa_mod);
	CODEGEN_PROFILE_TICK(6);
	...
	bool module_has_branchy_loop = false;
	for (int dfi = 0; dfi < ssa_mod.func_count; ++dfi) {
		SsaFunction *decision_func = ssa_mod.functions[dfi];
		for (int li = 0; li < decision_func->loop_count; ++li)
			if (decision_func->loops[li].member_count > 4)
				module_has_branchy_loop = true;
	...
	if (module_has_branchy_loop) {
		decision_plan.allow_vectorize = false;
		decision_plan.allow_unroll = false;
		decision_plan.allow_scalar_loop_optimization = false;
		decision_plan.allow_affine_recurrence = false;
		decision_plan.allow_magic_division = false;
		decision_plan.allow_loop_rotation = false;
	}
	decision_plan.allow_float_optimization =
		decision_plan.allow_float_optimization && ssa_module_has_float_ops(&ssa_mod);
	/* Current scalar-FP rewrites do not preserve a value converted back to an
	 * integer and consumed by integer loop/control-flow state.  Keep the fast
	 * pure-FP path, but conservatively skip it for mixed FP->integer programs. */
	for (int iri = 0; iri < cg->ir.text_count; ++iri) {
		if (cg->ir.text[iri].IrNode == IR_CVTTSD2SI) {
			decision_plan.allow_float_optimization = false;
			break;
		}
	}
	 ...
	 * ????????? mainCRTStartup????????? OS ??????????????
	 * ------------------------------------------------------------- */
	/* 入口代码无条件生成(不依赖 prog->main_block):
	 *   - 正常程序:main_block 存在,mira_main 由 SSA 定义 → 正常链接;
	 *   - 空 main:函数体为空被 SSA 剪掉,mira_main 无定义 → 链接器
	 *     报「unresolved symbol 'mira_main'」(与 Windows 原版一致);
	 *   - 无 main 程序:SSA 提供默认 mira_main 定义 → 正常链接,
	 *     入口等价于 Windows 侧的 rt_core main(固定退出码 0)。 */
	{
		/* 程序入口点:由 OS 加载器直接跳入,不是普通函数调用。
		 * 入口符号与退出序列随目标 ABI 不同:
		 *   Win64: mainCRTStartup -> call mira_main -> mov rcx,0; call ExitProcess
		 *   SysV:  _start -> (对齐栈) -> call mira_main -> mov edi,0; call exit
		 * 两种 ABI 都固定退出码 0:mira_main 的返回值(rax)不属于
		 * 语言语义,不构成进程退出码(与 Windows 原版行为严格一致)。
		 * SysV 下 _start 进入时 RSP 已 16 字节对齐(Glibc 约定),
		 * call mira_main 前需保持 16 字节对齐(call 会 push 8 字节返回地址,
		 * 所以 call 前 RSP 必须是 16 的倍数)。 */
		const char *entry = mira_abi_entry_symbol();
		ir_global(&cg->ir, entry);
		ir_label_named(&cg->ir, entry);

		if (mira_target_abi == MIRA_ABI_WIN64) {
			/* Win64: 32 字节影子空间 + 8 对齐 = sub rsp,40 */
			ir_sub_reg_imm(&cg->ir, REG_RSP, 40);
			ir_call_extern(&cg->ir, "mira_main");
			/* PE 入口不能直接 ret(会依赖未文档化的加载器状态),
			 * 用 ExitProcess 终止,RCX=0 表示退出码,保留已分配影子空间。 */
			ir_mov_reg_imm(&cg->ir, REG_RCX, 0);
			ir_call_extern(&cg->ir, mira_abi_exit_symbol());
		} else {
			/* SysV: 进程入口 RSP 由内核/加载器布置,标准 Linux 实测
			 * 16 字节对齐(≡ 0)。不依赖加载器行为,仿 GCC crt1.o 用
			 * and $-16,%rsp 强制对齐(入口 ≡ 8 时同样正确)。对齐后
			 * 直接 call:call 压 8 字节返回地址,使 mira_main 入口
			 * ≡ 8,满足"call 前 rsp ≡ 0 (mod 16)"约定。 */
			ir_and_reg_imm(&cg->ir, REG_RSP, -16);
			ir_call_extern(&cg->ir, "mira_main");
			/* 固定退出码 0(与 Win64 的 mov rcx,0 一致),不采用
			 * mira_main 的 rax 残留值作为进程退出码 */
			ir_mov_reg_imm(&cg->ir, REG_RDI, 0);
			/* 弹掉 call mira_main 压的返回地址,恢复 call 前对齐 */
			ir_add_reg_imm(&cg->ir, REG_RSP, 8);
			ir_call_extern(&cg->ir, mira_abi_exit_symbol());
		}
	}
	...
	extern void ir_opt_thread_jump_chains(IrBuffer *ir);
	ir_opt_thread_jump_chains(&cg->ir);
	ir_opt_remove_fallthrough_branches(&cg->ir);
	if (mira_opt_level >= 3)
		ir_opt_align_loop_headers(&cg->ir);
	CODEGEN_PROFILE_TICK(8);
	if (codegen_profile_enabled) {
		fprintf(stderr,
			"codegen-profile build=%.3f closure=%.3f ref=%.3f opt=%.3f "
			"phi-map=%.3f regalloc=%.3f lower=%.3f machine=%.3f total=%.3f\n",
			codegen_profile_ms(codegen_profile_ticks[0], codegen_profile_ticks[1]),
			codegen_profile_ms(codegen_profile_ticks[1], codegen_profile_ticks[2]),
			codegen_profile_ms(codegen_profile_ticks[2], codegen_profile_ticks[3]),
			codegen_profile_ms(codegen_profile_ticks[3], codegen_profile_ticks[4]),
			codegen_profile_ms(codegen_profile_ticks[4], codegen_profile_ticks[5]),
			codegen_profile_ms(codegen_profile_ticks[5], codegen_profile_ticks[6]),
			codegen_profile_ms(codegen_profile_ticks[6], codegen_profile_ticks[7]),
			codegen_profile_ms(codegen_profile_ticks[7], codegen_profile_ticks[8]),
			codegen_profile_ms(codegen_profile_ticks[0], codegen_profile_ticks[8]));
	}
#undef CODEGEN_PROFILE_TICK
}
