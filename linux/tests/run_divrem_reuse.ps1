...
function Get-ShapeSignedDivisionCount([int]$level, [bool]$disableMagic) {
    ...
    try {
        ...
        Push-Location $root
        try {
            $shapeDump = (& $Mira "-O$level" '--dump-asm' $shapeSource 2>&1 | Out-String)
        ...
        return [regex]::Matches($shapeDump, '(?i)\bidiv\s+').Count
    }
    finally {
...
foreach ($level in 0..3) {
    Push-Location $root
    try {
        $dump = (& $Mira "-O$level" '--dump-asm' $source 2>&1 | Out-String)
    }
    finally {
        Pop-Location
    }
