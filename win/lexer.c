...
void read_token(Token *t) {
	...
	/* :: is the modern typed-memory-place operator; a single : remains syntax punctuation. */
	if (s[0] == ':' && s[1] == ':') {
		t->kind = TOK_ID; t->len = 2; next_char(); next_char(); return;
	}
	/* : { } [ ] */
...
bool lexer_push_file(const char *path, const char *alias) {
	...
	comp->src = buf;
	comp->p = buf;
	size_t filename_size = strlen(path) + 1;
	char *owned_filename = (char *)malloc(filename_size);
	if (!owned_filename) { free(buf); return false; }
