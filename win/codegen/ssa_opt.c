...
static bool ssa_function_has_branchy_loop(const SsaFunction *func);
...
void ssa_decision_refresh_plans(SsaModule *mod, int optimization_level,
    ...
    for (int fi = 0; fi < mod->func_count; ++fi) {
        ...
        if (ssa_function_has_branchy_loop(func)) {
...
static int ssa_cse_operand_compare(SsaOperand a, SsaOperand b) {
    if (a.kind != b.kind) return a.kind < b.kind ? -1 : 1;
    switch (a.kind) {
        case SSA_OPND_NONE:
            return 0;
        case SSA_OPND_VREG:
            return a.u.vreg == b.u.vreg ? 0 : (a.u.vreg < b.u.vreg ? -1 : 1);
        case SSA_OPND_IMM:
            return a.u.imm == b.u.imm ? 0 : (a.u.imm < b.u.imm ? -1 : 1);
        case SSA_OPND_SYM:
            if (a.u.sym == b.u.sym) return 0;
            if (!a.u.sym) return -1;
            if (!b.u.sym) return 1;
            return strcmp(a.u.sym, b.u.sym);
        default:
            return 1;
    }
}

static bool ssa_cse_operand_equal(SsaOperand a, SsaOperand b) {
    return ssa_cse_operand_compare(a, b) == 0;
}

static bool ssa_cse_commutative(SsaOpcode op) {
    return op == SSA_OP_ADD || op == SSA_OP_MUL || op == SSA_OP_AND ||
        op == SSA_OP_OR || op == SSA_OP_XOR;
}

static bool ssa_cse_candidate(const SsaInst *inst) {
    if (!inst || inst->dst == 0 || inst->needs_free ||
        inst->type != SSA_TYPE_INT) return false;
    /* Function parameters are immutable.  The builder may emit one load for
     * each source-level use, so value-number them by parameter index too. */
    if (inst->IrNode == SSA_OP_LOAD_PARAM)
        return inst->op1.kind == SSA_OPND_IMM;
    return ssa_licm_pure_integer_op(inst) && inst->IrNode != SSA_OP_COPY;
}

static bool ssa_opt_local_cse(SsaFunction *func) {
    enum { SSA_CSE_MAX_ENTRIES = 256 };
    bool changed = false;
    if (!func) return false;

    for (int bi = 0; bi < func->block_count; ++bi) {
        SsaCseEntry entries[SSA_CSE_MAX_ENTRIES];
        int entry_count = 0;
        SsaBasicBlock *block = func->blocks[bi];
        for (SsaInst *inst = block ? block->inst_head : NULL; inst; inst = inst->next) {
            if (!ssa_cse_candidate(inst)) continue;

            SsaOperand a = inst->op1;
            SsaOperand b = inst->op2;
            if (ssa_cse_commutative(inst->IrNode) &&
                ssa_cse_operand_compare(a, b) > 0) {
                SsaOperand swap = a;
                a = b;
                b = swap;
            }
            ...
            for (int ei = 0; ei < entry_count; ++ei) {
                SsaCseEntry *entry = &entries[ei];
                if (entry->op == inst->IrNode && entry->type == inst->type &&
                    ssa_cse_operand_equal(entry->a, a) &&
                    ssa_cse_operand_equal(entry->b, b)) {
                    match = ei;
                    break;
                }
            }
            ...
            if (match >= 0) {
                inst->IrNode = SSA_OP_COPY;
                inst->op1.kind = SSA_OPND_VREG;
                inst->op1.u.vreg = entries[match].value;
                ...
                changed = true;
            } else if (entry_count < SSA_CSE_MAX_ENTRIES) {
                entries[entry_count].op = inst->IrNode;
                entries[entry_count].type = inst->type;
                entries[entry_count].a = a;
                entries[entry_count].b = b;
                entries[entry_count].value = inst->dst;
                ++entry_count;
            }
        }
    }
...
typedef struct {
    SsaCseEntry expression;
    SsaBasicBlock *block;
    SsaInst *definition;
} SsaGvnEntry;

static bool ssa_gvn_candidate(const SsaInst *inst) {
    if (!inst || inst->dst == 0 || inst->needs_free ||
        inst->type != SSA_TYPE_INT) return false;
    if (inst->IrNode == SSA_OP_LOAD_PARAM || inst->IrNode == SSA_OP_LOAD_VAR)
        return inst->op1.kind == SSA_OPND_IMM;
    if (ssa_licm_pure_integer_op(inst) && inst->IrNode != SSA_OP_COPY)
        return true;
    return inst->IrNode == SSA_OP_CMP_EQ || inst->IrNode == SSA_OP_CMP_NE ||
        inst->IrNode == SSA_OP_CMP_LT || inst->IrNode == SSA_OP_CMP_LE ||
        inst->IrNode == SSA_OP_CMP_GT || inst->IrNode == SSA_OP_CMP_GE;
}

static SsaOperand ssa_gvn_resolve_operand(const SsaFunction *func,
                                          SsaOperand operand,
                                          const VReg *aliases) {
    for (int budget = 0; func && operand.kind == SSA_OPND_VREG &&
         operand.u.vreg > 0 && operand.u.vreg < (VReg)func->vreg_defs_cap &&
         budget < 32; ++budget) {
        if (aliases && aliases[operand.u.vreg] > 0 &&
            aliases[operand.u.vreg] != operand.u.vreg) {
            operand.u.vreg = aliases[operand.u.vreg];
            continue;
        }
        SsaInst *def = func->vreg_defs ? func->vreg_defs[operand.u.vreg] : NULL;
...
static bool ssa_gvn_commutative(SsaOpcode op) {
    return ssa_cse_commutative(op) || op == SSA_OP_CMP_EQ ||
        op == SSA_OP_CMP_NE;
}

static void ssa_gvn_walk(SsaFunction *func, SsaBasicBlock *block,
                         SsaGvnEntry *table, int *table_count,
                         uint64_t *slot_versions,
                         VReg *aliases,
                         bool *visited, bool *changed) {
    enum { SSA_GVN_MAX_ENTRIES = 512, SSA_GVN_MAX_SLOTS = 128 };
    if (!func || !block || block->id < 0 || block->id >= func->block_count ||
        visited[block->id]) return;
    visited[block->id] = true;
    int rollback_count = *table_count;
    uint64_t saved_versions[SSA_GVN_MAX_SLOTS];
    memcpy(saved_versions, slot_versions, sizeof(saved_versions));

    /* A merge can receive a store from a sibling that is not on the
       dominator-tree walk.  Forget slot values there instead of guessing. */
    if (block != func->entry_block && block->pred_count != 1) {
        for (int slot = 0; slot < SSA_GVN_MAX_SLOTS; ++slot)
            ++slot_versions[slot];
    }

    for (SsaInst *inst = block->inst_head; inst; inst = inst->next) {
        if (inst->IrNode == SSA_OP_STORE_VAR) {
            if (inst->op2.kind == SSA_OPND_IMM && inst->op2.u.imm >= 0 &&
                inst->op2.u.imm < SSA_GVN_MAX_SLOTS) {
                int slot = (int)inst->op2.u.imm;
                ++slot_versions[slot];
                /* The stored SSA value is also the new value of this slot.
                   Seeding it here keeps forwarding alive when an earlier
                   same-block pass has already folded the first LOAD_VAR. */
                if (inst->op1.kind == SSA_OPND_VREG && func->vreg_defs &&
                    inst->op1.u.vreg > 0 &&
                    inst->op1.u.vreg < (VReg)func->vreg_defs_cap &&
                    func->vreg_defs[inst->op1.u.vreg] &&
                    *table_count < SSA_GVN_MAX_ENTRIES) {
                    SsaGvnEntry *entry = &table[(*table_count)++];
                    entry->expression.op = SSA_OP_LOAD_VAR;
                    entry->expression.type = SSA_TYPE_INT;
                    entry->expression.a = inst->op2;
                    entry->expression.b.kind = SSA_OPND_IMM;
                    entry->expression.b.u.imm =
                        (int64_t)slot_versions[slot];
                    entry->expression.value = inst->op1.u.vreg;
                    entry->block = block;
                    entry->definition = func->vreg_defs[inst->op1.u.vreg];
                }
            }
            continue;
        }
        ...
        if (!ssa_gvn_candidate(inst)) continue;
        SsaOperand a;
        SsaOperand b;
        if (inst->IrNode == SSA_OP_LOAD_VAR) {
            int64_t slot = inst->op1.u.imm;
            if (slot < 0 || slot >= SSA_GVN_MAX_SLOTS) continue;
            a = inst->op1;
            b.kind = SSA_OPND_IMM;
            b.u.imm = (int64_t)slot_versions[slot];
        } else {
            a = ssa_gvn_resolve_operand(func, inst->op1, aliases);
            b = ssa_gvn_resolve_operand(func, inst->op2, aliases);
        }
        if (ssa_gvn_commutative(inst->IrNode) &&
            ssa_cse_operand_compare(a, b) > 0) {
            SsaOperand swap = a;
            a = b;
            b = swap;
        ...
        int match = -1;
        for (int ei = *table_count - 1; ei >= 0; --ei) {
            SsaGvnEntry *entry = &table[ei];
            if (entry->expression.op != inst->IrNode ||
                entry->expression.type != inst->type ||
                !ssa_cse_operand_equal(entry->expression.a, a) ||
                !ssa_cse_operand_equal(entry->expression.b, b) ||
                !block_dominates(entry->block, block) ||
                !func->vreg_defs ||
                entry->expression.value >= (VReg)func->vreg_defs_cap ||
                func->vreg_defs[entry->expression.value] != entry->definition)
                continue;
            match = ei;
            break;
        }
        if (match >= 0) {
            if (inst->IrNode == SSA_OP_LOAD_VAR) {
                /* Preserve the slot load for lowering/register assignment,
                   but give later expressions the stored SSA value number. */
                aliases[inst->dst] = table[match].expression.value;
                *changed = true;
                continue;
            }
            inst->IrNode = SSA_OP_COPY;
            inst->op1.kind = SSA_OPND_VREG;
            inst->op1.u.vreg = table[match].expression.value;
            inst->op2.kind = SSA_OPND_NONE;
            if (inst->operands) {
                free(inst->operands);
                inst->operands = NULL;
            ...
            inst->operand_count = 1;
            *changed = true;
        } else if (*table_count < SSA_GVN_MAX_ENTRIES) {
            SsaGvnEntry *entry = &table[(*table_count)++];
            entry->expression.op = inst->IrNode;
            entry->expression.type = inst->type;
            entry->expression.a = a;
            entry->expression.b = b;
            entry->expression.value = inst->dst;
            entry->block = block;
            entry->definition = inst;
        }
    }

    for (int bi = 0; bi < func->block_count; ++bi) {
        SsaBasicBlock *child = func->blocks[bi];
        if (child && child != block && child->idom == block)
            ssa_gvn_walk(func, child, table, table_count, slot_versions,
                         aliases, visited, changed);
    }
    *table_count = rollback_count;
    memcpy(slot_versions, saved_versions, sizeof(saved_versions));
}

static bool ssa_opt_dominator_gvn(SsaFunction *func) {
    if (!func || !func->entry_block || func->block_count <= 0) return false;
    SsaGvnEntry table[512];
    uint64_t slot_versions[128] = {0};
    int table_count = 0;
    bool changed = false;
    bool *visited = calloc((size_t)func->block_count, sizeof(*visited));
    VReg *aliases = calloc((size_t)func->vreg_defs_cap, sizeof(*aliases));
    if (!visited || !aliases) {
        free(visited);
        free(aliases);
        return false;
    }
    ssa_gvn_walk(func, func->entry_block, table, &table_count, slot_versions,
                 aliases, visited, &changed);
    free(aliases);
    free(visited);
    return changed;
...
static bool ssa_value_depends_on_var_slot(const SsaFunction *func, VReg value,
                                          int slot, int budget) {
    if (!func || !value || budget <= 0 ||
        value >= (VReg)func->vreg_defs_cap) return false;
    SsaInst *def = func->vreg_defs[value];
    if (!def) return false;
    if (def->IrNode == SSA_OP_LOAD_VAR)
        return def->op1.kind == SSA_OPND_IMM &&
            (int)def->op1.u.imm == slot;
    switch (def->IrNode) {
        case SSA_OP_COPY:
        case SSA_OP_ADD: case SSA_OP_SUB: case SSA_OP_MUL:
        case SSA_OP_SDIV: case SSA_OP_SREM:
        case SSA_OP_NEG: case SSA_OP_NOT:
        case SSA_OP_AND: case SSA_OP_OR: case SSA_OP_XOR:
        case SSA_OP_SHL: case SSA_OP_ASHR: case SSA_OP_LSHR:
        case SSA_OP_CMP_EQ: case SSA_OP_CMP_NE:
        case SSA_OP_CMP_LT: case SSA_OP_CMP_LE:
        case SSA_OP_CMP_GT: case SSA_OP_CMP_GE:
            break;
        default:
            return false;
    }
    if (def->op1.kind == SSA_OPND_VREG &&
        ssa_value_depends_on_var_slot(func, def->op1.u.vreg, slot,
                                      budget - 1))
        return true;
    if (def->op2.kind == SSA_OPND_VREG &&
        ssa_value_depends_on_var_slot(func, def->op2.u.vreg, slot,
                                      budget - 1))
        return true;
    return false;
}

static bool ssa_loop_header_tests_slot(const SsaFunction *func,
                                       const SsaBasicBlock *header,
                                       int slot) {
    if (!func || !header || !header->inst_tail ||
        header->inst_tail->IrNode != SSA_OP_BR ||
        header->inst_tail->op1.kind != SSA_OPND_VREG)
        return false;
    return ssa_value_depends_on_var_slot(func,
        header->inst_tail->op1.u.vreg, slot, 32);
}
    ...
    for (int li = 0; li < func->block_count; ++li) {
        ...
        for (int bi = 0; bi < func->block_count; ++bi) {
            ...
            for (SsaInst *i = func->blocks[bi]->inst_head; i; i = i->next) {
                ...
                if (i->IrNode != SSA_OP_STORE_VAR || i->op1.kind != SSA_OPND_VREG ||
                    info->induction_slot >= 0)
                    continue;
                ...
                if (a && a->IrNode == SSA_OP_LOAD_VAR && (int)a->op1.u.imm == slot &&
                    ...
                    ssa_loop_header_tests_slot(func, header, slot)) {
...
static bool ssa_licm_group_operand_available(const SsaFunction *func,
                                             const SsaLoopInfo *loop,
                                             const SsaBasicBlock *preheader,
                                             const bool *group,
                                             SsaOperand operand) {
    if (operand.kind == SSA_OPND_NONE || operand.kind == SSA_OPND_IMM)
        return true;
    if (operand.kind != SSA_OPND_VREG || operand.u.vreg == 0 ||
        operand.u.vreg >= func->next_vreg)
        return false;
    if (group[operand.u.vreg]) return true;
    SsaInst *def = func->vreg_defs &&
        operand.u.vreg < (VReg)func->vreg_defs_cap ?
        func->vreg_defs[operand.u.vreg] : NULL;
    return def && def->parent && !loop->members[def->parent->id] &&
        block_dominates(def->parent, preheader);
}

static void ssa_licm_mark_group_uses(const SsaInst *inst, const bool *group,
                                     bool *boundary, VReg next_vreg) {
    SsaOperand direct[2] = { inst->op1, inst->op2 };
    for (int oi = 0; oi < 2; ++oi)
        if (direct[oi].kind == SSA_OPND_VREG &&
            direct[oi].u.vreg < next_vreg && group[direct[oi].u.vreg])
            boundary[direct[oi].u.vreg] = true;
    for (int oi = 0; inst->operands && oi < inst->operand_count; ++oi)
        if (inst->operands[oi].kind == SSA_OPND_VREG &&
            inst->operands[oi].u.vreg < next_vreg &&
            group[inst->operands[oi].u.vreg])
            boundary[inst->operands[oi].u.vreg] = true;
}

static bool ssa_licm_group_dependencies_ready(const SsaInst *inst,
                                              const bool *group,
                                              const bool *ready,
                                              VReg next_vreg) {
    SsaOperand direct[2] = { inst->op1, inst->op2 };
...
bool ssa_opt_var_forwarding(SsaFunction *func) {
    ...
    VReg slot_load[MAX_VAR_SLOTS]; /* repeated same-block load, killed by store/call */

    for (int b_idx = 0; b_idx < func->block_count; b_idx++) {
        SsaBasicBlock *b = func->blocks[b_idx];
        memset(slot_val, 0, sizeof(slot_val));
        memset(slot_load, 0, sizeof(slot_load));

        for (SsaInst *i = b->inst_head; i; i = i->next) {
            if (i->IrNode == SSA_OP_STORE_VAR) {
                int slot = (int)i->op2.u.imm;
                if (slot >= 0 && slot < MAX_VAR_SLOTS) slot_load[slot] = 0;
                ...
                if (slot >= 0 && slot < MAX_VAR_SLOTS &&
                    slot_load[slot] != 0 && i->dst > 0) {
                    i->IrNode = SSA_OP_COPY;
                    i->op1.kind = SSA_OPND_VREG;
                    i->op1.u.vreg = slot_load[slot];
                    i->op2.kind = SSA_OPND_NONE;
                    i->operand_count = 1;
                    any_changed = true;
                    continue;
                }
                if (slot >= 0 && slot < MAX_VAR_SLOTS && i->dst > 0)
                    slot_load[slot] = i->dst;
                if (func->var_reg_map && slot >= 0 && slot < func->var_count &&
                    func->var_reg_map[slot] != REG_NONE) {
                    continue;
                }
...
bool ssa_opt_licm(SsaFunction *func) {
    ...
    for (int loop_index = 0; loop_index < func->loop_count; ++loop_index) {
        ...
        for (int pi = 0; pi < func->loop_count; ++pi) {
            SsaLoopInfo *parent = &func->loops[pi];
            if (pi != loop_index && parent->members && parent->header &&
                parent->header != loop_header &&
                parent->members[loop_header->id] &&
                !loop->members[parent->header->id]) {
                nested_loop = true;
                break;
            }
        }
        ...
        bool loop_has_raw_memory = false;
        ...
        for (int k = 0; k < func->block_count; k++) {
            ...
            for (SsaInst *i = lb->inst_head; i; i = i->next) {
                ...
                if (i->IrNode == SSA_OP_LOAD || i->IrNode == SSA_OP_STORE ||
                    i->IrNode == SSA_OP_LOAD8 || i->IrNode == SSA_OP_STORE8 ||
                    i->IrNode == SSA_OP_GETELEMENTPTR)
                    loop_has_raw_memory = true;
            }
        }
        ...
        /* Pressure-budgeted nested LICM groups.  Only group outputs consumed
         * by instructions left in the loop extend a live range across it. */
        uint32_t pressure_budget = func->estimated_scalar_pressure < 10 ?
            10 - func->estimated_scalar_pressure : 0;
        bool *best_group = NULL;
        int best_boundary = INT_MAX, best_count = -1;
        if (nested_loop && !loop_has_raw_memory && !loop->has_unknown_call &&
            pressure_budget > 0 && func->next_vreg > 1 &&
            func->next_vreg <= 512 && func->block_count <= 64) {
            bool *group = calloc((size_t)func->next_vreg, sizeof(*group));
            bool *boundary = calloc((size_t)func->next_vreg, sizeof(*boundary));
            best_group = calloc((size_t)func->next_vreg, sizeof(*best_group));
            if (!group || !boundary || !best_group) {
                free(group); free(boundary); free(best_group);
                best_group = NULL;
            } else {
                int roots_seen = 0;
                for (int rbi = 0; rbi < func->block_count && roots_seen < 32; ++rbi) {
                    if (!loop->members[rbi]) continue;
                    for (SsaInst *root = func->blocks[rbi]->inst_head; root;
                         root = root->next) {
                        if (root->IrNode != SSA_OP_LOAD_VAR || root->dst == 0 ||
                            root->type != SSA_TYPE_INT || root->needs_free ||
                            root->op1.kind != SSA_OPND_IMM)
                            continue;
                        int slot = (int)root->op1.u.imm;
                        if (slot < 0 || slot >= MAX_VAR_SLOTS || slot_written[slot])
                            continue;
                        ++roots_seen;
                        memset(group, 0, (size_t)func->next_vreg * sizeof(*group));
                        memset(boundary, 0, (size_t)func->next_vreg * sizeof(*boundary));
                        group[root->dst] = true;
                        bool progress;
                        int closure_rounds = 0;
                        do {
                            progress = false;
                            for (int gbi = 0; gbi < func->block_count; ++gbi) {
                                if (!loop->members[gbi]) continue;
                                for (SsaInst *candidate=func->blocks[gbi]->inst_head;
                                     candidate; candidate=candidate->next) {
                                    if (candidate->dst == 0 ||
                                        group[candidate->dst] ||
                                        !ssa_licm_pure_integer_op(candidate))
                                        continue;
                                    bool depends_on_group =
                                        (candidate->op1.kind == SSA_OPND_VREG &&
                                         candidate->op1.u.vreg < func->next_vreg &&
                                         group[candidate->op1.u.vreg]) ||
                                        (candidate->op2.kind == SSA_OPND_VREG &&
                                         candidate->op2.u.vreg < func->next_vreg &&
                                         group[candidate->op2.u.vreg]);
                                    if (depends_on_group &&
                                        ssa_licm_group_operand_available(func,loop,
                                            preheader,group,candidate->op1) &&
                                        ssa_licm_group_operand_available(func,loop,
                                            preheader,group,candidate->op2)) {
                                        group[candidate->dst] = true;
                                        progress = true;
                                    }
                                }
                            }
                        } while (progress && ++closure_rounds < 8);
                        if (progress) continue;
                        int group_count = 0;
                        for (VReg v=1; v<func->next_vreg; ++v)
                            if (group[v]) ++group_count;
                        for (int ubi=0; ubi<func->block_count; ++ubi) {
                            if (!loop->members[ubi]) continue;
                            for (SsaInst *use=func->blocks[ubi]->inst_head; use;
                                 use=use->next)
                                if (use->dst == 0 || use->dst >= func->next_vreg ||
                                    !group[use->dst])
                                    ssa_licm_mark_group_uses(use,group,boundary,
                                                             func->next_vreg);
                        }
                        int boundary_count = 0;
                        for (VReg v=1; v<func->next_vreg; ++v)
                            if (boundary[v]) ++boundary_count;
                        if (boundary_count > 0 &&
                            boundary_count <= (int)pressure_budget &&
                            (boundary_count < best_boundary ||
                             (boundary_count == best_boundary &&
                              group_count > best_count))) {
                            memcpy(best_group,group,
                                   (size_t)func->next_vreg*sizeof(*group));
                            best_boundary=boundary_count;
                            best_count=group_count;
                        }
                    }
                }
                free(group); free(boundary);
            }
        }
        if (best_group && best_count > 0) {
            bool *ready=calloc((size_t)func->next_vreg,sizeof(*ready));
            SsaInst **order=calloc((size_t)best_count,sizeof(*order));
            int order_count=0;
            if (ready && order) {
                bool progress;
                do {
                    progress=false;
                    for (VReg v=1;v<func->next_vreg;++v) {
                        if (!best_group[v] || ready[v]) continue;
                        SsaInst *candidate=func->vreg_defs &&
                            v<(VReg)func->vreg_defs_cap?func->vreg_defs[v]:NULL;
                        if (candidate && ssa_licm_group_dependencies_ready(
                                candidate,best_group,ready,func->next_vreg)) {
                            ready[v]=true; order[order_count++]=candidate;
                            progress=true;
                        }
                    }
                } while(progress && order_count<best_count);
                if (order_count==best_count) {
                    for (int oi=0;oi<order_count;++oi) {
                        SsaInst *move=order[oi]; SsaBasicBlock *block=move->parent;
                        if (move->prev) move->prev->next=move->next;
                        else block->inst_head=move->next;
                        if (move->next) move->next->prev=move->prev;
                        else block->inst_tail=move->prev;
                        move->parent=preheader; move->next=term;
                        move->prev=term?term->prev:preheader->inst_tail;
                        if (term&&term->prev) term->prev->next=move;
                        else preheader->inst_head=move;
                        if (term) term->prev=move; else preheader->inst_tail=move;
                    }
                    any_changed=true;
                }
            }
            free(ready); free(order);
        }
        free(best_group);
        ...
        /* Scalar expression LICM used to hoist pure integer operations out of
         * arbitrary loop blocks. Branchy loops need a must-execute and
         * use-dominance proof before such motion is legal; without it, an
         * expression from one if/else arm can be executed every iteration.
         * Keep the conservative STORE_VAR/LOAD_PARAM paths above and leave
         * this broader expression motion disabled until that proof exists. */
        bool *stable = NULL;
        if (stable && !loop->has_unknown_call) {
            ...
            do {
                ...
                for (int bi = 0; bi < func->block_count; ++bi) {
                    ...
                    /* Hoisting from branch-only loop blocks is unsafe unless
                     * the candidate is proven to execute on every iteration.
                     * Keep this scalar LICM path to the loop header, whose
                     * instructions dominate the loop body and preserve the
                     * original control-flow execution frequency. */
                    if (func->blocks[bi] != loop_header) continue;
                    SsaBasicBlock *block = func->blocks[bi];
...
bool ssa_opt_induction_strength_reduce(SsaFunction *func) {
    ...
    for (int li = 0; li < func->loop_count; ++li) {
        ...
        if (!loop->members || !loop->header || !loop->latch ||
			...
			(func->decision_plan.pipeline.allow_vectorize &&
			 loop->memory_reads != 0 && !loop->has_unknown_call &&
			 !loop->has_ownership_transfer) ||
            loop->has_unknown_call || loop->has_ownership_transfer ||
            (loop->decision_plan.reference_vetoes &
             (DECISION_REF_VETO_MEMORY_REORDER |
              DECISION_REF_VETO_OWNERSHIP_MOVE)))
        {
            if (getenv("MIRA_DECISION_DEBUG"))
                fprintf(stderr,
                        "ssa-affine-skip function=%s phase=entry allow=%d veto=0x%x\n",
                        func->name ? func->name : "?",
                        func->decision_plan.pipeline.allow_affine_recurrence,
                        loop->decision_plan.reference_vetoes);
            continue;
        ...
        if (outside_preds != 1 || !preheader || !preheader->inst_tail) {
            if (getenv("MIRA_DECISION_DEBUG"))
                fprintf(stderr, "ssa-affine-skip function=%s phase=preheader preds=%d\n",
                        func->name ? func->name : "?", outside_preds);
            continue;
        }
        ...
        if (inside_stores != 1 || outside_stores != 1 ||
            ...
            if (getenv("MIRA_DECISION_DEBUG"))
                fprintf(stderr,
                        "ssa-affine-skip function=%s phase=stores inside=%d outside=%d initial_kind=%d update_kind=%d update_block=%d latch=%d\n",
                        func->name ? func->name : "?", inside_stores,
                        outside_stores,
                        initial_store ? (int)initial_store->op1.kind : -1,
                        update_store ? (int)update_store->op1.kind : -1,
                        update_store && update_store->parent ? update_store->parent->id : -1,
                        loop->latch ? loop->latch->id : -1);
            continue;
        }
        ...
        for (int bi = 0; bi < func->block_count; ++bi) {
            ...
            for (SsaInst *inst = func->blocks[bi]->inst_head; inst;
                ...
                if (getenv("MIRA_DECISION_DEBUG"))
                    fprintf(stderr,
                            "ssa-affine-mul function=%s block=%d dst=%u factor=%lld value=%u induction=%u def=%d slot=%lld\n",
                            func->name ? func->name : "?", bi, inst->dst,
                            (long long)current_factor, candidate_value,
                            induction_value,
                            candidate_def ? (int)candidate_def->IrNode : -1,
                            (long long)(candidate_def && candidate_def->op1.kind == SSA_OPND_IMM ?
                                        candidate_def->op1.u.imm : -1));
        ...
        if (getenv("MIRA_DECISION_DEBUG"))
            fprintf(stderr,
                    "ssa-affine-candidates function=%s groups=%d pressure=%u body=%u veto=0x%x\n",
                    func->name ? func->name : "?", group_count,
                    func->estimated_scalar_pressure,
                    loop->decision_plan.body_instructions,
                    loop->decision_plan.reference_vetoes);
        if (!group_count) continue;
        /* Every factor group adds one loop-carried recurrence.  Large loop
         * bodies are not a legality problem and are often where recurrence
         * replacement pays most; constrain the transform by measured scalar
         * pressure and the bounded group table instead of an arbitrary
         * instruction-count cutoff. */
        if (func->estimated_scalar_pressure >= 10) continue;
        int group_budget = 10 - func->estimated_scalar_pressure;
        if (group_budget > group_count) group_budget = group_count;
        /* Branch-heavy bodies need their remaining registers for transient
         * expression values.  Admit one best recurrence first; the candidate
         * ranking below chooses the factor removing the most multiplies. */
        if (loop->member_count > 4 && group_budget > 1) group_budget = 1;
        /* Spend scarce recurrence registers where they remove the most work.
         * This stable insertion sort preserves source order for equal-benefit
         * groups, so code generation stays deterministic. */
        for (int i = 1; i < group_count; ++i) {
            struct AffineGroup selected = groups[i];
            int j = i;
            while (j > 0 &&
                   groups[j - 1].candidate_count < selected.candidate_count) {
                groups[j] = groups[j - 1];
                --j;
            }
            groups[j] = selected;
        ...
        for (int gi = 0; gi < group_budget; ++gi) {
            ...
            /* Anchor the derived update to the unique proven induction store,
             * not to loop->latch: branchy canonical loops may place the
             * induction update in a merge block distinct from the block that
             * owns the backedge terminator. */
            SsaInst *after_induction_update = update_store->next;
            insert_inst_before(update_store->parent, after_induction_update,
                               derived_load);
            insert_inst_before(update_store->parent, after_induction_update,
                               derived_add);
            insert_inst_before(update_store->parent, after_induction_update,
...
void ssa_optimize_function(SsaFunction *func) {
    ...
    while (changed && pass < 20) {
        ...
        changed |= ssa_opt_local_cse(func);
    ...
    const char *gvn_fact_error = ssa_rebuild_function_facts(func);
    if (gvn_fact_error) return;
    if (ssa_opt_dominator_gvn(func)) {
        changed = true;
        while (changed) {
            changed = false;
            changed |= ssa_opt_copy_propagate(func);
...
static bool ssa_has_affine_terminal_shape(const SsaFunction *func) {
    if (!func) return false;
    for (int bi = 0; bi < func->block_count; ++bi)
        for (SsaInst *inst = func->blocks[bi]->inst_head; inst;
             inst = inst->next)
            if (inst->IrNode == SSA_OP_AND && inst->dst != 0 &&
                inst->op1.kind == SSA_OPND_VREG)
                return true;
    return false;
}
...
static bool ssa_function_has_branchy_loop(const SsaFunction *func) {
    if (!func) return false;
    for (int li = 0; li < func->loop_count; ++li)
        if (func->loops[li].member_count > 4)
            return true;
    return false;
}
...
static bool call_is_self1(const SsaInst *i, const char *name) {
    return i && i->IrNode == SSA_OP_CALL && i->operands &&
        i->operand_count == 2 && i->operands[0].kind == SSA_OPND_SYM &&
        i->operands[0].u.sym && strcmp(i->operands[0].u.sym, name) == 0 &&
        i->operands[1].kind == SSA_OPND_VREG;
}

static bool operand_is_vreg(SsaOperand op, VReg vreg) {
    return op.kind == SSA_OPND_VREG && op.u.vreg == vreg;
}

static bool load_is_param0(const SsaInst *i) {
    return i && i->IrNode == SSA_OP_LOAD_PARAM &&
        i->op1.kind == SSA_OPND_IMM && i->op1.u.imm == 0;
}

static SsaInst *new_simple_inst(SsaOpcode op, SsaType type, VReg dst,
                                SsaOperand a, SsaOperand b,
                                SsaBasicBlock *parent) {
    SsaInst *i = calloc(1, sizeof(*i));
    if (!i) return NULL;
    i->IrNode = op; i->type = type; i->dst = dst;
    i->op1 = a; i->op2 = b; i->parent = parent;
    return i;
}

static void insert_before_term(SsaBasicBlock *b, SsaInst *i) {
    SsaInst *term = b->inst_tail;
    i->next = term;
    i->prev = term ? term->prev : NULL;
    if (i->prev) i->prev->next = i; else b->inst_head = i;
    if (term) term->prev = i; else b->inst_tail = i;
}

static void remove_edge_once(SsaBasicBlock *from, SsaBasicBlock *to) {
    for (int i = 0; i < from->succ_count; ++i) if (from->succs[i] == to) {
        memmove(from->succs + i, from->succs + i + 1,
                (size_t)(from->succ_count - i - 1) * sizeof(*from->succs));
        from->succ_count--; break;
    }
    for (int i = 0; i < to->pred_count; ++i) if (to->preds[i] == from) {
        memmove(to->preds + i, to->preds + i + 1,
                (size_t)(to->pred_count - i - 1) * sizeof(*to->preds));
        to->pred_count--; break;
    }
}

/* Loopify one branch of f(n)=base-or-(f(n-a)+f(n-b)).  Exact structural
 * matching keeps the rewrite side-effect free and constant-size. */
static bool loopify_additive_recursion(SsaFunction *f) {
    if (!f || !f->name || f->param_count != 1 || f->block_count != 4) return false;
    SsaBasicBlock *h=f->blocks[0], *base=f->blocks[1], *rec=f->blocks[2], *exit=f->blocks[3];
    SsaInst *hp=h->inst_head, *bp=base->inst_head, *x=rec->inst_head;
    if (!load_is_param0(hp) || !hp->next || hp->next->IrNode!=SSA_OP_IMM ||
        !hp->next->next || hp->next->next->IrNode!=SSA_OP_CMP_LT ||
        !h->inst_tail || h->inst_tail->IrNode!=SSA_OP_BR ||
        !load_is_param0(bp) || !base->inst_tail ||
        base->inst_tail->IrNode!=SSA_OP_JMP) return false;
    SsaInst *limit=hp->next, *cmp=limit->next;
    if (!operand_is_vreg(cmp->op1,hp->dst) || !operand_is_vreg(cmp->op2,limit->dst) ||
        h->succ_count!=2 || base->pred_count!=1 || rec->pred_count!=1 ||
        exit->pred_count!=2 || h->succs[0]!=base || h->succs[1]!=rec ||
        base->inst_tail->op1.kind!=SSA_OPND_BLOCK ||
        base->inst_tail->op1.u.block!=exit) return false;
    SsaInst *k1=x?x->next:NULL, *sub1=k1?k1->next:NULL, *c1=sub1?sub1->next:NULL;
    SsaInst *p2=c1?c1->next:NULL, *k2=p2?p2->next:NULL, *sub2=k2?k2->next:NULL;
    SsaInst *c2=sub2?sub2->next:NULL, *sum=c2?c2->next:NULL, *jmp=sum?sum->next:NULL;
    if (!load_is_param0(x) || !k1 || k1->IrNode!=SSA_OP_IMM ||
        k1->op1.kind!=SSA_OPND_IMM || k1->op1.u.imm<=0 || !sub1 || sub1->IrNode!=SSA_OP_SUB ||
        !operand_is_vreg(sub1->op1,x->dst) || !operand_is_vreg(sub1->op2,k1->dst) ||
        !call_is_self1(c1,f->name) || !operand_is_vreg(c1->operands[1],sub1->dst) ||
        !load_is_param0(p2) ||
        !k2 || k2->IrNode!=SSA_OP_IMM || k2->op1.kind!=SSA_OPND_IMM || k2->op1.u.imm<=0 ||
        !sub2 || sub2->IrNode!=SSA_OP_SUB ||
        !operand_is_vreg(sub2->op1,p2->dst) || !operand_is_vreg(sub2->op2,k2->dst) ||
        !call_is_self1(c2,f->name) || !operand_is_vreg(c2->operands[1],sub2->dst) ||
        !sum || sum->IrNode!=SSA_OP_ADD || sum->op1.kind!=SSA_OPND_VREG ||
        sum->op2.kind!=SSA_OPND_VREG || sum->op1.u.vreg!=c1->dst || sum->op2.u.vreg!=c2->dst ||
        !jmp || jmp!=rec->inst_tail || jmp->IrNode!=SSA_OP_JMP ||
        jmp->op1.kind!=SSA_OPND_BLOCK || jmp->op1.u.block!=exit) return false;
    SsaInst *phi=exit->inst_head;
    if (!phi || phi->IrNode!=SSA_OP_PHI || !phi->operands || phi->operand_count!=4 ||
        phi->operands[0].kind!=SSA_OPND_VREG || phi->operands[0].u.vreg!=bp->dst ||
        phi->operands[1].kind!=SSA_OPND_BLOCK || phi->operands[1].u.block!=base ||
        phi->operands[2].kind!=SSA_OPND_VREG || phi->operands[2].u.vreg!=sum->dst ||
        phi->operands[3].kind!=SSA_OPND_BLOCK || phi->operands[3].u.block!=rec ||
        !phi->next || phi->next!=exit->inst_tail || phi->next->IrNode!=SSA_OP_RET ||
        !operand_is_vreg(phi->next->op1,phi->dst)) return false;

    SsaBasicBlock *pre=ssa_create_block(f,"recurrence_preheader");
    VReg init_n=ssa_new_vreg(f,SSA_TYPE_INT), init_a=ssa_new_vreg(f,SSA_TYPE_INT);
    VReg acc=ssa_new_vreg(f,SSA_TYPE_INT), base_v=ssa_new_vreg(f,SSA_TYPE_INT);
    VReg next_a=ssa_new_vreg(f,SSA_TYPE_INT);
    SsaOperand none={0}, imm0={.kind=SSA_OPND_IMM,.u.imm=0};
    SsaInst *ln=new_simple_inst(SSA_OP_LOAD_PARAM,SSA_TYPE_INT,init_n,imm0,none,pre);
    SsaInst *za=new_simple_inst(SSA_OP_IMM,SSA_TYPE_INT,init_a,imm0,none,pre);
    SsaInst *pj=new_simple_inst(SSA_OP_JMP,SSA_TYPE_VOID,0,
        (SsaOperand){.kind=SSA_OPND_BLOCK,.u.block=h},none,pre);
    SsaInst *ap=new_simple_inst(SSA_OP_PHI,SSA_TYPE_INT,acc,none,none,h);
    SsaInst *ba=new_simple_inst(SSA_OP_ADD,SSA_TYPE_INT,base_v,
        (SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=bp->dst},
        (SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=acc},base);
    SsaInst *na=new_simple_inst(SSA_OP_ADD,SSA_TYPE_INT,next_a,
        (SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=acc},
        (SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=c1->dst},rec);
    if (!pre || !ln || !za || !pj || !ap || !ba || !na) return false;
    ln->next=za; za->prev=ln; za->next=pj; pj->prev=za; pre->inst_head=ln; pre->inst_tail=pj;
    hp->IrNode=SSA_OP_PHI; hp->op1=none; hp->op2=none;
    hp->operand_count=hp->operand_cap=4; hp->operands=calloc(4,sizeof(*hp->operands));
    ap->operand_count=ap->operand_cap=4; ap->operands=calloc(4,sizeof(*ap->operands));
    if (!hp->operands || !ap->operands) return false;
    hp->operands[0]=(SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=init_n};
    hp->operands[1]=(SsaOperand){.kind=SSA_OPND_BLOCK,.u.block=pre};
    hp->operands[2]=(SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=sub2->dst};
    hp->operands[3]=(SsaOperand){.kind=SSA_OPND_BLOCK,.u.block=rec};
    ap->operands[0]=(SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=init_a};
    ap->operands[1]=(SsaOperand){.kind=SSA_OPND_BLOCK,.u.block=pre};
    ap->operands[2]=(SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=next_a};
    ap->operands[3]=(SsaOperand){.kind=SSA_OPND_BLOCK,.u.block=rec};
    ap->prev=hp; ap->next=hp->next; hp->next->prev=ap; hp->next=ap;
    for(SsaInst *i=bp;i;i=i->next) if(i->IrNode==SSA_OP_LOAD_PARAM){i->IrNode=SSA_OP_COPY;i->op1=(SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=hp->dst};i->operand_count=1;}
    for(SsaInst *i=rec->inst_head;i;i=i->next) if(i->IrNode==SSA_OP_LOAD_PARAM){i->IrNode=SSA_OP_COPY;i->op1=(SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=hp->dst};i->operand_count=1;}
    insert_before_term(base,ba); insert_before_term(rec,na);
    c2->prev->next=sum->next; sum->next->prev=c2->prev;
    f->vreg_defs[c2->dst]=NULL; f->vreg_defs[sum->dst]=NULL;
    free(c2->operands); free(c2); free(sum);
    jmp->op1=(SsaOperand){.kind=SSA_OPND_BLOCK,.u.block=h};
    free(phi->operands); phi->operands=NULL; phi->operand_count=phi->operand_cap=0;
    phi->IrNode=SSA_OP_COPY; phi->op1=(SsaOperand){.kind=SSA_OPND_VREG,.u.vreg=base_v}; phi->op2=none;
    remove_edge_once(rec,exit); ssa_add_edge(rec,h); ssa_add_edge(pre,h);
    for(int i=f->block_count-1;i>0;--i) f->blocks[i]=f->blocks[i-1];
    f->blocks[0]=pre; for(int i=0;i<f->block_count;++i) f->blocks[i]->id=i; f->entry_block=pre;
    f->vreg_defs[init_n]=ln; f->vreg_defs[init_a]=za; f->vreg_defs[acc]=ap;
    f->vreg_defs[base_v]=ba; f->vreg_defs[next_a]=na;
    return true;
}
...
void ssa_optimize_module(SsaModule *mod) {
	...
	if (mira_opt_level >= 3)
		for (int fi = 0; fi < mod->func_count; ++fi)
			loopify_additive_recursion(mod->functions[fi]);
    ...
    if (mira_opt_level >= 3) {
        ...
        for (int fi = 0; fi < mod->func_count; ++fi) {
            ...
            if (!ssa_function_needs_preinline_cleanup(func)) continue;
    ...
    /* Rebuild once after CFG/DCE rewrites.  Affine collapse does not change
     * CFG edges, so these current facts remain valid after a committed rewrite. */
    for (int fi = 0; fi < mod->func_count; ++fi) {
        SsaFunction *func = mod->functions[fi];
        const char *reason = ssa_rebuild_function_facts(func);
        ssa_debug_ssa_skip(func, "post-opt", reason);
        if (reason || mira_opt_level < 3 ||
            !func->decision_plan.pipeline.allow_affine_collapse ||
            !ssa_has_affine_terminal_shape(func)) continue;
            ssa_analyze_loops(func);
            bool affine_changed = ssa_opt_affine_collapse(func);
            if (affine_changed) {
    ...
    if (mira_opt_level >= 3)
        for (int fi = 0; fi < mod->func_count; ++fi) {
            if (!ssa_function_has_signed_div(mod->functions[fi])) continue;
...
bool ssa_opt_inline(SsaModule *mod) {
    ...
    if (!was_inlined || !ssa_function_index_rebuild_call_facts(mod)) {
        free(was_inlined);
        return false;
    }
    ...
    for (int fi = 0; fi < mod->func_count && budget_used < INLINE_MAX_BUDGET; fi++) {
        ...
        for (int bi = 0; bi < caller->block_count && budget_used < INLINE_MAX_BUDGET; bi++) {
            ...
            for (SsaInst *inst = bb->inst_head; inst && budget_used < INLINE_MAX_BUDGET; ) {
                ...
                SsaFunction *callee = ssa_function_index_find(mod, callee_name);
                ...
                int callee_index = ssa_function_index_ordinal(mod, callee);
                if (callee_index < 0) {
                    inst = next_inst;
                    continue;
                }
                ...
                int multi_site_tiny_leaf =
                    ssa_function_index_direct_calls(mod, callee) > 1 &&
                    ssa_function_index_direct_calls(mod, callee) <= 8 &&
                    callee_size <= 8 &&
                    !func_has_any_call(callee);
                if (ssa_function_index_direct_calls(mod, callee) != 1 && !multi_site_tiny_leaf) {
                    inst = next_inst;
                    continue;
                }
    ...
    ssa_function_index_rebuild_call_facts(mod);
    bool removed_function = false;
    for (int ci = original_func_count - 1; ci >= 0; ci--) {
        ...
        bool referenced = ssa_function_index_is_referenced(mod, candidate);
        if (!referenced) {
    ...
    if (removed_function) {
        ...
        ssa_function_index_rebuild(mod);
    } else {
