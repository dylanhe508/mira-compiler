...
if ($Group -in @('all', 'ownership')) {
    ...
    $ownershipSsaExe = Join-Path $types 'ownership_ssa_metadata_test.exe'
    ...
    $ownershipBuilderExe = Join-Path $types 'ownership_ssa_builder_test.exe'
    ...
    try {
        ...
        foreach ($level in @(0, 3)) {
            ...
            $actual = ((& (Join-Path $types 'checker_ownership_flow_valid.exe')) -join "`n").Trim()
...
if ($Group -in @('all', 'declarations')) {
    ...
    $lexerStateExe = Join-Path $types 'lexer_state_test.exe'
...
if ($Group -in @('all', 'calls')) {
    ...
    try {
        ...
        $actual = ((& (Join-Path $types 'method_arity_valid.exe')) -join "`n").Trim()
    ...
    try {
        ...
        foreach ($case in $focusedCases) {
            foreach ($level in @(0, 3)) {
                ...
                $actual = ((& (Join-Path $types "$($case.Name).exe")) -join "`n").Trim()
