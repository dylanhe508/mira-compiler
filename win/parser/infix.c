...
static IrNode *parse_eval_block(Program *prog) {
	...
	while (!lexer_at(TOK_EOF)) {
		if (lexer_at(TOK_INT) || lexer_at(TOK_FLOAT) || lexer_at(TOK_STR) ||
			...
			Token *t = lexer_cur();
			if (!comp->current_is_stdlib && t->kind == TOK_ID && t->len >= 7 && memcmp(t->start, "__mira_", 7) == 0)
				mira_error(comp->src, comp->filename, t->line, t->col, 1,
					"reserved runtime symbol '%.*s' is not available to user code",
					(int)t->len, t->start);
			...
			if (prec > 0) { // It's an operator (+, -, etc.)
				...
				if (dot && dot != t->start && dot + 1 < t->start + t->len) {
					...
					if (!owner) {
						...
						int module_error = 0;
						char *qualified = module_resolve_dotted(&comp->modules,
							comp->current_module, t->start, t->len, &qualified_len, &module_error);
						if (!qualified)
							mira_error(comp->src, comp->filename, t->line, t->col, 1,
								"unknown module alias '%.*s'", (int)receiver_len, t->start);
						int found_member = 0;
						for (Def *d = prog->defs; d; d = d->next)
							if (d->name_len == qualified_len && memcmp(d->name, qualified, qualified_len) == 0)
								found_member = 1;
						if (!found_member)
							mira_error(comp->src, comp->filename, t->line, t->col, 1,
					...
					Token call = *t;
					if (comp->current_module != 0 && !is_mira_builtin(t->start, t->len)) {
						char *qualified = module_qualify_symbol(&comp->modules,
							comp->current_module, t->start, t->len);
						size_t qualified_len = strlen(qualified);
						for (Def *d = prog->defs; d; d = d->next) {
							if (d->name_len == qualified_len &&
							    memcmp(d->name, qualified, qualified_len) == 0) {
								call.start = qualified;
								call.len = qualified_len;
								break;
							}
						}
					}
...
static IrNode *parse_infix_nonlogical(Program *prog) {
	IrNode *out_queue_head = NULL;
	IrNode *out_queue_tail = NULL;
			...
			for (;;) {
				...
				if (!arg)
					mira_error(comp->src, comp->filename, lexer_cur()->line,
						lexer_cur()->col, 1, "function argument expects an expression");
				push_out_line(arg);
				if (!lexer_at(TOK_COMMA)) break;
				lexer_advance();
			}
		}
		lexer_expect(TOK_RPAREN);
	...
	while (!lexer_at(TOK_EOF)) {
		...
		/* Recursive call-argument parsing owns its comma and closing paren. */
		if (paren_depth == 0 && (lexer_at(TOK_COMMA) || lexer_at(TOK_RPAREN)))
			break;
		...
		if (lexer_at(TOK_INT) || lexer_at(TOK_FLOAT) || lexer_at(TOK_STR) ||
			...
			lexer_advance();
			IrNode *group = parse_infix_line(prog);
			if (!group)
				mira_error(comp->src, comp->filename, lexer_cur()->line,
					lexer_cur()->col, 1, "parenthesized expression is empty");
			if (!lexer_at(TOK_RPAREN))
				mira_error(comp->src, comp->filename, lexer_cur()->line,
					lexer_cur()->col, 1, "mismatched parentheses in infix expression");
			lexer_advance();
			push_out_line(group);
			if (!expect_operand) postfix_seen = 1;
			expect_operand = 0;
			saw_operand = 1;
		} else if (lexer_at(TOK_RPAREN)) {
			paren_depth--;
			...
			Token *t = lexer_cur();
			/* Logical operators are lowered structurally by parse_infix_line.
			 * Stop before consuming a top-level operator so the RHS remains
			 * unevaluated until its control-flow branch is selected. */
			if (paren_depth == 0 && ((t->len == 2 && memcmp(t->start, "&&", 2) == 0) ||
			    (t->len == 2 && memcmp(t->start, "||", 2) == 0))) break;
			/* A typed memory place reads in value context.  i64 maps to the
			 * backend's existing word-sized memory operation, preserving all
			 * Static Reference ownership and alias facts below the parser. */
			if (t->len == 2 && memcmp(t->start, "::", 2) == 0) {
				if (expect_operand || !saw_operand)
					mira_error(comp->src, comp->filename, t->line, t->col, 1,
						"'::' expects an address expression on the left");
				lexer_advance();
				int is_i64 = lexer_at(TOK_ID) && lexer_cur()->len == 3 &&
					memcmp(lexer_cur()->start, "i64", 3) == 0;
				int is_u8 = lexer_at(TOK_ID) && lexer_cur()->len == 2 &&
					memcmp(lexer_cur()->start, "u8", 2) == 0;
				if (!is_i64 && !is_u8)
					mira_error(comp->src, comp->filename, lexer_cur()->line,
						lexer_cur()->col, 1,
						"memory place type must be 'i64' or 'u8'");
				lexer_advance();
				IrNode *load = new_ir(IR_WORD);
				load->u.word.name = is_u8 ? "m8@" : "m64@";
				load->u.word.len = is_u8 ? 3 : 4;
				push_out_line(load);
				expect_operand = 0;
				saw_operand = 1;
				continue;
			...
			if (joined_negative) {
				...
				saw_operand = 1;
				continue;
			}			if (expect_operand && t->len >= 2 && t->start[0] == '-' &&
			    (isalpha((unsigned char)t->start[1]) || t->start[1] == '_')) {
				IrNode *zero = new_ir(IR_INT);
				zero->u.i = 0;
				push_out_line(zero);
				int value_slot = prog_var_slot(prog, t->start + 1, t->len - 1);
				IrNode *value = value_slot >= 0 ? new_ir(IR_VAR) : new_ir(IR_WORD);
				if (value_slot >= 0) {
					value->u.var_slot = value_slot;
					IrNode *fetch = new_ir(IR_WORD);
					fetch->u.word.name = "@";
					fetch->u.word.len = 1;
					value->next = fetch;
				} else {
					/* Parameters are resolved by name during code generation rather
					 * than occupying parser variable slots. Preserve the identifier
					 * so param_slot() can resolve it normally. */
					value->u.word.name = t->start + 1;
					value->u.word.len = t->len - 1;
				}
				push_out_line(value);
				IrNode *minus = new_ir(IR_WORD);
				minus->u.word.name = "-";
				minus->u.word.len = 1;
				push_out_line(minus);
				lexer_advance();
...
static IrNode *logical_booleanize(IrNode *value) {
...
static IrNode *logical_block(IrNode *body) {
	IrNode *block = new_ir(IR_BLOCK);
	block->u.block = body;
	return block;
}

static IrNode *logical_constant(int64_t value) {
	IrNode *node = new_ir(IR_INT);
	node->u.i = value;
	return logical_block(node);
}

static IrNode *parse_logical_and(Program *prog) {
	IrNode *left = parse_infix_nonlogical(prog);
	while (lexer_at(TOK_ID) && lexer_cur()->len == 2 &&
	       memcmp(lexer_cur()->start, "&&", 2) == 0) {
		lexer_advance();
		IrNode *right = parse_infix_nonlogical(prog);
		if (!right)
			mira_error(comp->src, comp->filename, lexer_cur()->line,
				lexer_cur()->col, 1, "'&&' expects a right operand");
		IrNode *logical = new_ir(IR_IF);
		logical->u.iff.cond = left;
		logical->u.iff.then_b = logical_block(logical_booleanize(right));
		logical->u.iff.else_b = logical_constant(0);
		left = logical;
	}
	return left;
}
...
static IrNode *parse_infix_line(Program *prog) {
	IrNode *left = parse_logical_and(prog);
	while (lexer_at(TOK_ID) && lexer_cur()->len == 2 &&
	       memcmp(lexer_cur()->start, "||", 2) == 0) {
		lexer_advance();
		IrNode *right = parse_logical_and(prog);
		if (!right)
			mira_error(comp->src, comp->filename, lexer_cur()->line,
				lexer_cur()->col, 1, "'||' expects a right operand");
		IrNode *logical = new_ir(IR_IF);
		logical->u.iff.cond = left;
		logical->u.iff.then_b = logical_constant(1);
		logical->u.iff.else_b = logical_block(logical_booleanize(right));
		left = logical;
	}
	return left;
}
